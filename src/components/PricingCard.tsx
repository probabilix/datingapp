...
const PricingCard: React.FC<PricingCardProps> = ({ name, price, period, features, isPopular, isCurrent, onSelect, buttonText, disabled }) => (
    <div className={`relative bg-white p-10 rounded-[3rem] border-2 transition-all hover:shadow-2xl flex flex-col ${isPopular ? 'border-[#E94057] scale-105 z-10' : 'border-transparent shadow-sm'}`}>
        ...
        <button
            ...
            onClick={onSelect}
            className={`w-full py-4 rounded-2xl font-bold transition-all active:scale-95 shadow-lg ${(isCurrent || disabled) ? 'bg-gray-100 text-gray-400 cursor-not-allowed shadow-none' : 'bg-[#E94057] text-white hover:brightness-110 cursor-pointer'}`}
        >
